...
program
    .version('1.0.0')
    .description('This script generates images using the FAL AI serverless client. You can provide custom prompts, select models, and adjust settings to customize the image generation process.')
    .option('--prompt <text>', 'Specify the text prompt for image generation. If omitted, a random prompt from "prompts.txt" is used.')
    .option('--model <modelKey>', 'Choose the AI model to use.')
    .option('--format <formatKey>', 'Specify image size/format.')
    .option('--lora <loraKeys>', 'Apply one or more LoRAs (comma-separated).', (value, previous) => previous ? previous.concat(value.split(',').map(s => s.trim())) : value.split(',').map(s => s.trim()))
    .option('--seed <number>', 'Set a seed for randomization to reproduce results.')
    .option('--scale <number>', 'Set the scale for guidance.')
    .option('--out', 'Save images to the current directory instead of the default.')
    .option('--index <number>', 'Use a specific prompt from "prompts.txt" by line number.')
    ...
    if (allPrompts) {
        ...
        getAllPrompts(promptsFilePath).then(async prompts => {
            for (const [idx, promptText] of prompts.entries()) {
                ...
                await run(promptText, modelEndpoint, pictureFormat, loraObjects, seed, scale);
            }
        }).catch(error => {
